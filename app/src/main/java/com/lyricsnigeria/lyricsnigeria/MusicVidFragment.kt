...
class MusicVidFragment : Fragment() {

    private var mDatabase: DatabaseReference? = null
    private var recyclerView: RecyclerView? = null

    private var adapter: MusicVid? = null

    private var adLoader: AdLoader? = null
    private var mNativeAds = ArrayList<UnifiedNativeAd?>()
    private var mRecyclerViewItems = ArrayList<Any?>()
    private var AD_UNIT_ID: String = "ca-app-pub-5242580240582734/1324650188"

    private val mWaitHandler = Handler()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        val x = inflater.inflate(R.layout.fragment_music_vid, container, false)

        mDatabase = FirebaseDatabase.getInstance().reference.child("Videos").child("Music")

        recyclerView = x.findViewById(R.id.mvid_recycler_view)
        recyclerView!!.setHasFixedSize(true)
        recyclerView!!.layoutManager = LinearLayoutManager(this@MusicVidFragment.context)

        mDatabase!!.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                mRecyclerViewItems.clear()
                for (dataSnapshot1 in dataSnapshot.children) {
                    val p = dataSnapshot1.getValue(MusicVid.VideoAdapter::class.java)
                    mRecyclerViewItems.add(p!!)
                }
                adapter = MusicVid(this@MusicVidFragment.context!!, mRecyclerViewItems)
    ...
    private fun loadNativeAds(){
        if (isNetworkAvailable()){
            val builder = AdLoader.Builder(this@MusicVidFragment.context!!, AD_UNIT_ID)
