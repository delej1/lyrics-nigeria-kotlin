...
class RandomVidFragment : Fragment() {

    private var mDatabase: DatabaseReference? = null
    private var recyclerView: RecyclerView? = null

    private var adapter: RandomVid? = null

    private var adLoader: AdLoader? = null
    private var mNativeAds = ArrayList<UnifiedNativeAd?>()
    private var mRecyclerViewItems = ArrayList<Any?>()
    private var AD_UNIT_ID: String = "ca-app-pub-5242580240582734/1484911219"

    private val mWaitHandler = Handler()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        val x = inflater.inflate(R.layout.fragment_random_vid, container, false)

        mDatabase = FirebaseDatabase.getInstance().reference.child("Videos").child("Random")

        recyclerView = x.findViewById(R.id.rvid_recycler_view)
        recyclerView!!.setHasFixedSize(true)
        recyclerView!!.layoutManager = LinearLayoutManager(this@RandomVidFragment.context)

        mDatabase!!.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                mRecyclerViewItems.clear()
                for (dataSnapshot1 in dataSnapshot.children) {
                    val p = dataSnapshot1.getValue(RandomVid.VideoAdapter::class.java)
                    mRecyclerViewItems.add(p!!)
                }
                adapter = RandomVid(this@RandomVidFragment.context!!, mRecyclerViewItems)
    ...
    private fun loadNativeAds(){
        if (isNetworkAvailable()){
            val builder = AdLoader.Builder(this@RandomVidFragment.context!!, AD_UNIT_ID)
