...
class FavouriteLyrics : AppCompatActivity(), View.OnClickListener, Player.EventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_favourite_lyrics)

        mAuth = FirebaseAuth.getInstance()

        mDatabase = FirebaseDatabase.getInstance().reference.child("Favourites").child(mAuth!!.currentUser!!.uid)

        mLyricsKey = intent.extras!!.getString("Lyrics_id")

        mCover = findViewById(R.id.cover_favourite_lyrics)
        mArtist = findViewById(R.id.artist_favourite_lyrics)
        mLyrics = findViewById(R.id.favourites_lyrics_body)
        mSong = findViewById(R.id.song_favourite_lyrics)
        mUploader = findViewById(R.id.uploader_favourite_lyrics)
        mBeat = findViewById(R.id.beat_favourite_activity)
        mPlayBtn = findViewById(R.id.fav_play_btn)
        mStopBtn = findViewById(R.id.fav_stop_btn)
        mPauseBtn = findViewById(R.id.fav_pause_btn)
        mRewindBtn = findViewById(R.id.fav_rewind_btn)
        mForwardBtn = findViewById(R.id.fav_fast_btn)
        mRepeatBtn = findViewById(R.id.fav_restart_btn)
        simpleSeekBar = findViewById(R.id.fav_exo_seekbar)
        mCurrentTime = findViewById(R.id.fav_current_time_text)
        mDuration = findViewById(R.id.fav_duration_text)

        revealBtn = findViewById(R.id.fav_reveal_btn)
        concealBtn = findViewById(R.id.fav_conceal_btn)
        shareBtn = findViewById(R.id.fav_share_btn)
        btnCase = findViewById(R.id.fav_record_btn_case)

        recordBtn = findViewById(R.id.fav_record_btn)
        stopRecordBtn = findViewById(R.id.fav_stop_record_btn)
        playRecordBtn = findViewById(R.id.fav_play_record_btn)
        stopPlayRecordBtn = findViewById(R.id.fav_stop_play_record_btn)
    ...
    private fun recordAudio() {//create function to record audio
        try {
            ...
            Toasty.normal(this@FavouriteLyrics, "Recording Audio").show()
    ...
    private fun stopRecord() {//stop recording
        try {
            if (isRecording) {
                ...
                Toasty.normal(this@FavouriteLyrics, "Recording Saved").show()
            } else {
                recordBtn!!.isEnabled = true
                stopRecordBtn!!.isEnabled = false
                playRecordBtn!!.isEnabled = false
                Toasty.normal(this@FavouriteLyrics, "Not recording anything").show()
            }
        } catch (e: IllegalStateException) {
            e.printStackTrace()
        } catch (e: IOException) {
        ...
        when (view!!.id) {
            ...
            R.id.fav_play_btn -> {
                ...
                if (isNetworkAvailable() && !(exoPlayer != null && exoPlayer!!.playWhenReady) && mBeat!!.text.toString().isNotEmpty() && !isPaused) {
                    ...
                    isPaused = false
                    Toasty.normal(this@FavouriteLyrics, "Streaming Instrumental").show()
                } else {
                    if (!isNetworkAvailable()) {
                        Toasty.normal(this@FavouriteLyrics, "No Internet Connection").show()
                    } else {
                        if (isNetworkAvailable() && mBeat!!.text.toString().isEmpty()) {
                            Toasty.normal(this@FavouriteLyrics, "No Media to Stream").show()
                        } else {
                            if (isPaused) {
                                exoPlayer!!.playWhenReady = true
                                mPlayBtn!!.isVisible = false
            ...
            R.id.fav_stop_record_btn -> {
                stopRecord()
            }
            R.id.fav_play_record_btn -> {
                playAudio()
            }
            R.id.fav_stop_play_record_btn -> {
                stopAudio()
            }
            R.id.fav_share_btn -> {
