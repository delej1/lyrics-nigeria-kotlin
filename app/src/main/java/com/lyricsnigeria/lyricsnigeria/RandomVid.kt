...
class RandomVid(context: Context, recyclerViewItems: ArrayList<Any?>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var mRecyclerViewItems: ArrayList<Any?> = recyclerViewItems
    private var mDatabase: DatabaseReference? = null
    private var mContext: Context? = context
    private val TYPE_RVID: Int = 0
    private val TYPE_AD: Int = 1

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == TYPE_RVID) {
            val rvidItemLayoutView = LayoutInflater.from(parent.context).inflate(
                    R.layout.video_listitem, parent, false) as YouTubePlayerView
            RandomVidFragment().lifecycle.addObserver(rvidItemLayoutView)
            return VideoAdapter.VideoViewHolder(rvidItemLayoutView)
        } else {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        } else if (viewType == TYPE_RVID) {
            val rvidItem: VideoAdapter = mRecyclerViewItems[position] as VideoAdapter
            (holder as VideoAdapter.VideoViewHolder).vidTitle!!.text = rvidItem.title
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    for (childSnapshot in dataSnapshot.children) {
                        ...
                        if (title == holder.vidTitle!!.text) {
                            run {
                                holder.cueVideo(rvidItem.videoId!!)
                            }
                        }
                    }
                }
