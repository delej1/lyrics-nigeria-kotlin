...
class LyricsVid(context: Context, recyclerViewItems: ArrayList<Any?>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var mRecyclerViewItems: ArrayList<Any?> = recyclerViewItems
    private var mDatabase: DatabaseReference? = null
    private var mContext: Context? = context
    private val TYPE_LVID: Int = 0
    private val TYPE_AD: Int = 1

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == TYPE_LVID) {
            val lvidItemLayoutView = LayoutInflater.from(parent.context).inflate(
                    R.layout.video_listitem, parent, false) as YouTubePlayerView
            LyricsVidFragment().lifecycle.addObserver(lvidItemLayoutView)
            return VideoAdapter.VideoViewHolder(lvidItemLayoutView)
        } else {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        } else if (viewType == TYPE_LVID) {
            val lvidItem: VideoAdapter = mRecyclerViewItems[position] as VideoAdapter
            (holder as VideoAdapter.VideoViewHolder).vidTitle!!.text = lvidItem.title
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    for (childSnapshot in dataSnapshot.children) {
                        ...
                        if (title == holder.vidTitle!!.text) {
                            run {
                                holder.cueVideo(lvidItem.videoId!!)
                            }
                        }
                    }
                }
