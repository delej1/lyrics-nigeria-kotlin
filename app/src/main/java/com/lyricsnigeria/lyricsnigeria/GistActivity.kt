...
class GistActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mGistKey = intent.extras!!.getString("Gist_id")
        ...
        mDatabase!!.child(mGistKey!!).addValueEventListener(object : ValueEventListener {//set lyrics body to firebase database value
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                val mc = MediaController(this@GistActivity)//calling media controller for pause and rewind function
                ...
                vidLayout!!.setOnClickListener {
                    if (!isPlaying && isNetworkAvailable()) {
                        ...
                        mVideo!!.start()
                        isPlaying = true
                        Toasty.normal(this@GistActivity, "Streaming Video").show()
                    } else {
                        if (isPlaying) {
                            mVideo!!.stopPlayback()
                            mVideo!!.clearFocus()
                            isPlaying = false
                            Toasty.normal(this@GistActivity, "Video Stopped").show()
                        } else {
                            if (!isNetworkAvailable()) {
                                Toasty.normal(this@GistActivity, "No Internet Connection").show()
                            }
                        }
                    }
                }
